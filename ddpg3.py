...
class DDPG(object):
    def __init__(self, a_dim, s_dim, a_bound,):
        ...
        with tf.variable_scope("ema"):
            a_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='Sr_Actor')
            c_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='Sr_Critic')
        ...
        with tf.variable_scope("sr_a_train"):
            self.atrain = tf.train.AdamOptimizer(LR_A).minimize(a_loss, var_list=a_params)

        with tf.control_dependencies(target_update):    # soft replacement happened at here
            with tf.variable_scope("q_target"):
                q_target = self.R + GAMMA * q_
            with tf.variable_scope("sr_td_error"):
                td_error = tf.losses.mean_squared_error(labels=q_target, predictions=q)
            with tf.variable_scope("sr_c_train"):
    ...
    def _build_a(self, s, reuse=None, custom_getter=None):
        ...
        with tf.variable_scope('Sr_Actor', reuse=reuse, custom_getter=custom_getter):
    ...
    def _build_c(self, s, a, reuse=None, custom_getter=None):
        trainable = True if reuse is None else False
        with tf.variable_scope('Sr_Critic', reuse=reuse, custom_getter=custom_getter):
            n_l1 = 128
