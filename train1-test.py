...
def train(arglist):
    ...
    checkpoint = tf.train.get_checkpoint_state("train1/model/")
    if checkpoint and checkpoint.model_checkpoint_path:
        saver.restore(sess, './train1/model/-730')
...
#        if terminal and (len(episode_rewards) % arglist.save_rate == 2):
#            savefliename=filename+"/"+arglist.save_dir
#            os.makedirs(os.path.dirname(savefliename), exist_ok=True)
#            saver.save(sess, savefliename, global_step= len(episode_rewards)-1)
